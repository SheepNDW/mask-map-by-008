...
const currStore = computed(() => {
  return store.filteredStores.filter((item) => item.id === infoBoxSid.value)[0];
});
...
console.log(servicePeriods.value);
</script>
          ...
          <div class="modal-body" v-if="currStore">
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <th>早上</th>
                  <td v-for="(item, index) in servicePeriods[0]" :key="index">
                    {{ item }}
                  </td>
                </tr>
                <tr>
                  <th>中午</th>
                  <td v-for="(item, index) in servicePeriods[1]" :key="index">
                    {{ item }}
                  </td>
                </tr>
                <tr>
                  <th>晚上</th>
                  <td v-for="(item, index) in servicePeriods[2]" :key="index">
                    {{ item }}
                  </td>
                </tr>
              </tbody>
